...
-- if i want descending order than name desc

-- Grades in History for Simon
SELECT  grade FROM grades WHERE studentID = 1 AND subjectID =1;

-- List all student names that haven't taken any classes (don't have grade in grades table)
...
-- List all teachers that haven't yet taught any class
...
SELECT name FROM teachers WHERE tid IN (SELECT teacherID FROM grades WHERE  subjectID IN (SELECT subid FROM
	subjects WHERE name = 'Biology')
);
